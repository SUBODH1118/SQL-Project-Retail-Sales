...
-- Q.2 Write Sale of all indivisual Category
...
-- Q.4 Find average quantity sold per transaction for each product category.
...
-- Q.14 Calculate gender-wise contribution to total sales (percentage).
-- Q.15 Write a SQL query to find the number of unique customers who purchased items from each category.
-- Q.16 Find the age group (e.g., 18–25, 26–35, 36–45, etc.) that generates the highest revenue.
-- Q.17 Write a SQL query to create each shift and number of orders (Example Morning <=12, Afternoon Between 12 & 17, Evening >17)
-- Q.18 Determine the customer with the highest lifetime value.
-- Q.19 Find average total sale value per transaction.
-- Q.20 Identify the top 5 highest-value transactions.
...
SELECT 
    ...
    RANK() OVER(PARTITION BY EXTRACT(YEAR FROM sale_date) ORDER BY AVG(total_sale) DESC) as Ranked FROM retail_sales
GROUP BY 1, 2
) as t1
WHERE ranked = 1
